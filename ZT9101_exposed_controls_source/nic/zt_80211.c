                                ...
                                zt_80211_mgmt_ie_t **pout)
{
    ...
    while (offset + 2 <= var_len)
    {
        pie = (zt_80211_mgmt_ie_t *)(pstart + offset);
        if ((zt_u16)(offset + 2 + pie->len) > var_len)
            return ZT_RETURN_FAIL;
        ...
        zt_80211_mgmt_ie_t **ppie)
{
    ...
    if (pies == NULL || oui == NULL || ppie == NULL)
        return ZT_RETURN_FAIL;

    while (offset + 2 <= ies_len)
    {
        pie = (zt_80211_mgmt_ie_t *)(pstart + offset);
        if ((zt_u16)(offset + 2 + pie->len) > ies_len)
            return ZT_RETURN_FAIL;

        if (pie->element_id == cmp_id && pie->len >= 4 &&
                zt_memcmp(pie->data, oui, 4) == 0)
        {
            *ppie = pie;
            return ZT_RETURN_OK;
        }
                               ...
                               zt_u32 *pgroup_cipher, zt_u32 *pairwise_cipher)
{
    ...
    pos = pie->data;
    /* The caller may pass a buffer containing more IEs.  Parse only this IE,
     * otherwise a short WPA/RSN IE can make us consume the next IE and report
     * bogus/corrupt security data. */
    left = pie->len;
