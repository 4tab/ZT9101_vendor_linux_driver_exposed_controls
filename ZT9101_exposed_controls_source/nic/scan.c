...
#define SCAN_ACTIVE_PROBE           LOCAL_INFO->scan_active
...
zt_s32 zt_scan_probe_send(nic_info_st *pnic_info)
{
    ...
    /*
     * A USB surprise removal can race the scan worker.  Do not construct or
     * queue another management frame after the HIF has already gone away.
     */
    if (pnic_info->is_surprise_removed || pnic_info->is_driver_stopped ||
            pnic_info->hif_node == NULL)
    {
        SCAN_WARN("scan probe skipped: device is stopped/removed");
        ...
        ZT_OFFSETOF(zt_80211_mgmt_t, probe_req.variable) + var_len;
    {
        ...
        tx_ret = zt_nic_mgmt_frame_xmit(pnic_info, NULL, pxmit_buf,
                                        pxmit_buf->pkt_len);
        if (tx_ret)
        {
            SCAN_WARN("probe frame send fail, tx_ret=%d", tx_ret);
            return tx_ret;
        }
    }
...
zt_s32 check_channel(zt_scan_info_t *pscan_info, zt_u8 *pies, zt_u16 ies_len)
{
    ...
    {
        if (pie->len < sizeof(zt_80211_mgmt_dsss_parameter_t))
        {
            return -2;
        }
                      ...
                      zt_80211_mgmt_t *pmgmt, zt_u16 mgmt_len)
{
    ...
    {
        zt_u16 ie_offset = ZT_OFFSETOF(struct beacon_ie, variable);
        zt_u8 *pies;
        zt_u16 ies_len;

        if (mgmt_len < ie_offset)
        {
            rst = -6;
            goto exit;
        }
        pies = &pmgmt->probe_resp.variable[0];
        ies_len = mgmt_len - ie_offset;
...
zt_pt_ret_t zt_sta_scan_thrd(zt_pt_t *pt, nic_info_st *pnic_info, zt_s32 *prsn)
{
    ...
    SCAN_DBG();
    SCAN_INFO("scan config: active=%u ch_to=%ums probe_times=%u queue_depth=%u ttl=%u",
              LOCAL_INFO->scan_active, LOCAL_INFO->scan_ch_to,
              LOCAL_INFO->scan_prb_times, LOCAL_INFO->scan_que_deep,
              LOCAL_INFO->scan_que_node_ttl);
            ...
            pscan_info->ch_idx++)
    {
        ...
        /*
         * This USB device/firmware combination is unstable when broadcast
         * probe requests are transmitted during a scan: the host observes a
         * real USB disconnect immediately after a few probe frames.  Keep
         * cfg80211's requested scan semantics, but allow the configuration
         * to select a receive-only (passive) scan so NetworkManager can
         * discover normal beaconing APs without exercising the crashing TX
         * path.
         */
        if (pscan_info->preq->type == SCAN_TYPE_ACTIVE && SCAN_ACTIVE_PROBE)
        {
            SCAN_DBG("active probe TX enabled");
                    ...
                    pscan_info->retry_cnt++)
            {
                ...
                /* Do not stack management URBs on this legacy USB firmware. */
                while (!is_tx_empty(pnic_info) && !zt_timer_expired(&pscan_info->timer))
                {
                    PT_YIELD(pt);
                }
                if (zt_timer_expired(&pscan_info->timer))
                {
                    SCAN_WARN("probe TX queue did not drain before scan probe");
                    reason = -7;
                    goto exit;
                }
        ...
        {
            /* receive-only scan: dwell for beacons on the selected channel */
    ...
    /* refresh scan queue only while the HIF is still alive. */
    if (!ZT_CANNOT_RUN(pnic_info))
    {
    ...
    if (!ZT_CANNOT_RUN(pnic_info) && !zt_p2p_is_valid(pnic_info) &&
...
exit:
    ...
    /* Never touch the HIF again after surprise removal/disconnect. */
    if (!ZT_CANNOT_RUN(pnic_info))
    {
                     ...
                     zt_u8 chs[], zt_u8 ch_num)
{
    ...
    /* debug dump */
    {
        ...
        zt_s8 *ssid_str = zt_kzalloc(33 * ZT_SCAN_REQ_SSID_NUM + 1);
        zt_s8 *ch_str = zt_kzalloc(4 * ZT_SCAN_REQ_CHANNEL_NUM + 1);
        zt_u8 i;
        /* bssid */
        ...
        if (ssids)
        {
            for (i = 0; i < ZT_MIN(ssid_num, ZT_SCAN_REQ_SSID_NUM); i++)
            {
                zt_strncat(ssid_str, (const zt_s8 *)&ssids[i].data,
                           ZT_MIN(ssids[i].length, 32));
            }
        }
        else
        {
        ...
        if (chs)
        {
            for (i = 0; i < ZT_MIN(ch_num, (zt_u8)ZT_SCAN_REQ_CHANNEL_NUM); i++)
            {
                zt_s8 tmp[5];
    ...
    if (chs && pscan_req->ch_num)
    {
        zt_memcpy(pscan_req->ch_map, chs, pscan_req->ch_num);
    }
