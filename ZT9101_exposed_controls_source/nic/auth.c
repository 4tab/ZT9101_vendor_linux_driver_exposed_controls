                        ...
                        zt_80211_statuscode_e status)
{
    ...
    /* Open-system authentication is a fixed 30-byte 802.11 management frame.
     * Keep the length bounded and make the frame contents explicit before the
     * USB management-TX path sees it. */
    if (frame_len < ZT_OFFSETOF(zt_80211_mgmt_t, auth.variable) ||
            frame_len > sizeof(zt_80211_mgmt_t))
    {
        AUTH_WARN("invalid auth frame length: %u", frame_len);
        zt_xmit_extbuf_delete(ptx_info, pxmit_buf);
        return -5;
    }

    pxmit_buf->pkt_len = frame_len;
    AUTH_DBG("auth TX seq=%u alg=%u len=%u da="ZT_MAC_FMT,
             pwdn_info->auth_seq, pwdn_info->auth_algo, frame_len,
             ZT_MAC_ARG(pauth_frame->da));
