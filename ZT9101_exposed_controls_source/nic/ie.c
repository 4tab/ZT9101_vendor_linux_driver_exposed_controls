...
static void zt_ie_apply_wireless_mode(nic_info_st *nic_info, wdn_net_info_st *wdn_info)
{
    local_info_st *local_info = nic_info->local_info;
    zt_u8 allowed = local_info->wireless_mode;

    if (allowed == WIRELESS_AUTO)
        return;

    wdn_info->network_type &= allowed;
}
                                    ...
                                    wdn_net_info_st *wdn_info, zt_u8 *pie_data, zt_u8 len)
{
    ...
    zt_ie_apply_wireless_mode(nic_info, wdn_info);
    return ZT_RETURN_OK;
}
...
//    do_set_mcs_by_flag(wdn_info->ht_cap.supp_mcs_set, MCS_RATE_1R);
    ...
    /* Optional test/debug ceiling for the 1-stream HT MCS range.
     * 255 keeps the AP-advertised MCS set unchanged. */
    if (local_info->ht_mcs_max <= 7)
    {
        zt_u8 mcs = local_info->ht_mcs_max;
        zt_u8 i;

        /* Keep only MCS values that the AP already advertised, then apply
         * the configured ceiling.  Do not manufacture unsupported MCS bits. */
        for (i = (zt_u8)(mcs + 1); i < 8; i++)
            wdn_info->ht_cap.supp_mcs_set[0] &= (zt_u8)~ZT_BIT(i);
        for (i = 1; i < ZT_MCS_NUM; i++)
            wdn_info->ht_cap.supp_mcs_set[i] = 0;
        LOG_I("HT MCS ceiling configured: MCS0-MCS%d", mcs);
    }
