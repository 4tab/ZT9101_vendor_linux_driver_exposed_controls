...
zt_s32 zt_local_cfg_set_default(nic_info_st *nic_info)
{
    ...
    /* Re-apply the selected calibrated RF table after channel/RF calibration
     * and mode setup.  Those operations may touch the same RF state. */
...
zt_u8 zt_local_cfg_get_wireless_mode(nic_info_st *pnic_info)
{
    ...
    return plocal ? plocal->wireless_mode : WIRELESS_AUTO;
}
...
{
    local_info_st *plocal;
    zt_s8 txpower_dbm;
    zt_u8 rf_power;
    zt_u8 vco_cur;
    zt_u8 txpower_cfg_set;
    zt_u16 rts_thresh;
    zt_u16 frag_thresh;
    zt_u8 vcs_en;
    zt_u8 vcs_type;
    ...
    plocal = (local_info_st *)pnic_info->local_info;
    if (plocal == NULL)
        return;

    /*
     * Changing the nl80211 interface type must not silently reset runtime
     * RF configuration.  The old implementation copied the complete mode
     * template here, which also reset txpower_dbm/rf_power/vco_cur.  That
     * could make a successful `iw set txpower` appear to revert to 0 dBm.
     */
    txpower_dbm = plocal->txpower_dbm;
    rf_power = plocal->rf_power;
    vco_cur = plocal->vco_cur;
    txpower_cfg_set = plocal->txpower_cfg_set;
    rts_thresh = plocal->rts_thresh;
    frag_thresh = plocal->frag_thresh;
    vcs_en = plocal->vcs_en;
    vcs_type = plocal->vcs_type;
    tx_data_rpt = plocal->tx_data_rpt;
    sm_ps_support = plocal->sm_ps_support;
    wdn_sleep_support = plocal->wdn_sleep_support;
    ldpc_support = plocal->ldpc_support;
    tx_stbc_support = plocal->tx_stbc_support;
    max_rx_ampdu_factor = plocal->max_rx_ampdu_factor;
    best_ampdu_density = plocal->best_ampdu_density;
    ht_mcs_max = plocal->ht_mcs_max;
    wireless_mode = plocal->wireless_mode;
    sgi_20 = plocal->sgi_20;
    sgi_40 = plocal->sgi_40;

    zt_memcpy(plocal, &default_cfg[mode], sizeof(local_info_st));

    plocal->txpower_dbm = txpower_dbm;
    plocal->rf_power = rf_power;
    plocal->vco_cur = vco_cur;
    plocal->txpower_cfg_set = txpower_cfg_set;
    plocal->rts_thresh = rts_thresh;
    plocal->frag_thresh = frag_thresh;
    plocal->vcs_en = vcs_en;
    plocal->vcs_type = vcs_type;
    plocal->tx_data_rpt = tx_data_rpt;
    plocal->sm_ps_support = sm_ps_support;
    plocal->wdn_sleep_support = wdn_sleep_support;
    plocal->ldpc_support = ldpc_support;
    plocal->tx_stbc_support = tx_stbc_support;
    plocal->max_rx_ampdu_factor = max_rx_ampdu_factor;
    plocal->best_ampdu_density = best_ampdu_density;
    plocal->ht_mcs_max = ht_mcs_max;
    plocal->wireless_mode = wireless_mode;
    plocal->sgi_20 = sgi_20;
    plocal->sgi_40 = sgi_40;
}
