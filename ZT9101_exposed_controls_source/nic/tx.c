                                  ...
                                  zt_bool wait_ack)
{
    ...
    /* Management frames must always use the management queue and a 1 Mbps
     * base rate for legacy control/association traffic.  Keep the descriptor
     * deterministic so stale bits from a reused xmit buffer cannot leak into
     * firmware. */
    zt_set_bits_to_le_u32(pbuf + 8, 18, 7,
                          zt_p2p_is_valid(nic_info) ? DESC_RATE6M : DESC_RATE1M);
    zt_set_bits_to_le_u32(pbuf + 12, 6, 5, 0);
