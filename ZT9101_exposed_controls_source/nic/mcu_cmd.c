...
/*                  0x0b, 0x0b, 0x0b, 0x0b,
                    ...
                    0x0e, 0x0e, 0x0c, 0x0c,
                    0x0e, 0x0e, 0x0a, 0x0a, 0x08, 0x08, 0x06, 0x00,
                    0x0e, 0x0e, 0x0a, 0x0a, 0x08, 0x08, 0x06, 0x00,
                },
                .cal = local_info->vco_cur,
            },
    ...
    /*
     * Diagnostic visibility for cfg80211/wifi.cfg TX-power requests.
     *
     * cfg80211 expresses power in dBm, but this chipset consumes one of
     * three calibrated RF tables.  Dump the exact payload selected here so
     * that an iw set-txpower request can be verified all the way to the MCU.
     * Keep this at INFO level: it only runs when the RF table is programmed.
     */
    LOG_I("[%s] txpower=%d dBm rf_table=%u driver_flag=%u vco_cur=0x%02x",
          __func__, (int)local_info->txpower_dbm, (unsigned int)rf_pwr_ofs,
          (unsigned int)hif_node->drv_ops->driver_flag,
          (unsigned int)pwr[hif_node->drv_ops->driver_flag][rf_pwr_ofs].cal);
    LOG_I("[%s] rf_ofs b=%02x %02x %02x %02x g=%02x %02x %02x %02x %02x %02x %02x %02x",
          __func__,
          pwr[hif_node->drv_ops->driver_flag][rf_pwr_ofs].ofs[0],
          pwr[hif_node->drv_ops->driver_flag][rf_pwr_ofs].ofs[1],
          pwr[hif_node->drv_ops->driver_flag][rf_pwr_ofs].ofs[2],
          pwr[hif_node->drv_ops->driver_flag][rf_pwr_ofs].ofs[3],
          pwr[hif_node->drv_ops->driver_flag][rf_pwr_ofs].ofs[4],
          pwr[hif_node->drv_ops->driver_flag][rf_pwr_ofs].ofs[5],
          pwr[hif_node->drv_ops->driver_flag][rf_pwr_ofs].ofs[6],
          pwr[hif_node->drv_ops->driver_flag][rf_pwr_ofs].ofs[7],
          pwr[hif_node->drv_ops->driver_flag][rf_pwr_ofs].ofs[8],
          pwr[hif_node->drv_ops->driver_flag][rf_pwr_ofs].ofs[9],
          pwr[hif_node->drv_ops->driver_flag][rf_pwr_ofs].ofs[10],
          pwr[hif_node->drv_ops->driver_flag][rf_pwr_ofs].ofs[11]);
    LOG_I("[%s] rf_ofs n=%02x %02x %02x %02x %02x %02x %02x %02x",
          __func__,
          pwr[hif_node->drv_ops->driver_flag][rf_pwr_ofs].ofs[12],
          pwr[hif_node->drv_ops->driver_flag][rf_pwr_ofs].ofs[13],
          pwr[hif_node->drv_ops->driver_flag][rf_pwr_ofs].ofs[14],
          pwr[hif_node->drv_ops->driver_flag][rf_pwr_ofs].ofs[15],
          pwr[hif_node->drv_ops->driver_flag][rf_pwr_ofs].ofs[16],
          pwr[hif_node->drv_ops->driver_flag][rf_pwr_ofs].ofs[17],
          pwr[hif_node->drv_ops->driver_flag][rf_pwr_ofs].ofs[18],
          pwr[hif_node->drv_ops->driver_flag][rf_pwr_ofs].ofs[19]);
