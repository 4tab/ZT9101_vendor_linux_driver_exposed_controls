...
zt_s32 zt_hw_info_get_default_cfg(nic_info_st *pnic_info)
{
    ...
    phw_info->rts_thresh = local_info->rts_thresh;
    phw_info->frag_thresh = local_info->frag_thresh;
    phw_info->vcs_en = local_info->vcs_en;
    phw_info->vcs_type = local_info->vcs_type;

    phw_info->use_fixRate = zt_false;

    phw_info->tx_data_rpt = local_info->tx_data_rpt;
    ...
    phw_info->sm_ps_support = local_info->sm_ps_support;
    phw_info->max_rx_ampdu_factor = local_info->max_rx_ampdu_factor;
    phw_info->wdn_sleep_support = local_info->wdn_sleep_support;
    phw_info->ldpc_support = local_info->ldpc_support;
    phw_info->rx_stbc_support = zt_true;
    phw_info->rx_stbc_num = 1;
    phw_info->tx_stbc_support = local_info->tx_stbc_support;
    phw_info->rx_packet_offset = RXDESC_SIZE + DRVINFO_SZ * 8;
    phw_info->max_recvbuf_sz = MAX_RECVBUF_SZ;
    phw_info->best_ampdu_density = local_info->best_ampdu_density;
    phw_info->rf_type = 3;
