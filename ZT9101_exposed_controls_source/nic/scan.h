...
typedef struct
{
    ...
    zt_u8 ssid_num;
    /* cfg80211/P2P can supply more than the 2.4 GHz hardware channel count.
     * Keep the request container large enough; the actual hardware channel
     * list is still enforced by hw_info/channelplan. */
    zt_u8 ch_map[ZT_SCAN_REQ_CHANNEL_NUM];
    zt_u8 ch_num;
} zt_scan_req_t;
