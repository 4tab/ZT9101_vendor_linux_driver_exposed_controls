...
{
    /*
     * Treat an over-full queue as full as well.  Equality is unsafe because
     * corruption/races can leave cnt above the nominal capacity, after which
     * the old check allowed another remove from an already empty free queue.
     */
    return (zt_que_count(queue) >= queue_node_num);
}
                                  ...
                                  void *param), void *tx_info, void *param)
{
    ...
    {
        /*
         * The caller owns the buffer until the queue accepts it.  Returning
         * here without invoking the callback leaked xmit_extbuf objects.
         * During repeated scans that leak eventually makes
         * zt_xmit_extbuf_new() fail and the probe request can never be sent.
         */
        LOG_W("[%s] tx_queue is full (tx=%d free=%d)",
              __func__, tx_queue->cnt, free_tx_queue->cnt);
    ...
    if (qnode == NULL)
    {
        LOG_E("get node from free_tx_queue error free_tx_queue_cnt[%d] tx_queue_cnt[%d]",
              free_tx_queue->cnt, tx_queue->cnt);
        if (tx_callback_func)
        {
            tx_callback_func(tx_info, param);
        }
