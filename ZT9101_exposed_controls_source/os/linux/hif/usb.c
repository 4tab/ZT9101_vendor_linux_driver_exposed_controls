...
static inline void usb_write_port_complete(struct urb *purb)
{
    ...
    else
    {
        ...
        /*
         * The TX callback owns/reclaims the xmit buffer.  The old error path
         * returned the queue node without calling it, leaking xmit_extbuf
         * objects whenever an URB failed.  Repeated scan/recovery cycles could
         * then exhaust the management TX buffers.
         */
        if (qnode->tx_callback_func)
        {
            qnode->tx_callback_func(qnode->tx_info, qnode->param);
        }

        if (purb->status == -ENODEV || purb->status == -ESHUTDOWN ||
                purb->status == -ENOENT)
        {
            hnode->dev_removed = zt_true;
            if (hnode->nic_info[0])
                hnode->nic_info[0]->is_driver_stopped = zt_true;
        }
