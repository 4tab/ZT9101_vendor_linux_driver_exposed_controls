...
static zt_s32 wiphy_cfg(struct wiphy *pwiphy)
{
    ...
    pwiphy->interface_modes = ZT_BIT(NL80211_IFTYPE_STATION)
                              ...
                              | ZT_BIT(NL80211_IFTYPE_MESH_POINT)
#endif
#if defined(ZT_CONFIG_P2P) && ((LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 37)) || defined(COMPAT_KERNEL_RELEASE))
                              | ZT_BIT(NL80211_IFTYPE_P2P_CLIENT)
                              | ZT_BIT(NL80211_IFTYPE_P2P_GO)
                                     ...
                                     struct vif_params *params)
{
    ...
    switch (type)
    {
        ...
        case NL80211_IFTYPE_MESH_POINT:
            /* The chip firmware has no native 802.11s op-mode.  Use the
             * existing IBSS engine as the data-plane compatibility path,
             * while exposing the Linux interface as NL80211 mesh. */
            network_type = ZT_ADHOC_MODE;
            break;
#endif
...
#ifdef CFG_ENABLE_ADHOC_MODE
/*
 * Linux 802.11s entry points.  The ZT9101 firmware does not implement a
 * native mesh MCU mode, so these callbacks deliberately reuse the driver's
 * mature IBSS engine.  This makes `iw dev X set type mesh` and `iw dev X mesh
 * join ...` usable on Linux without sending the unsupported ZT_MESH_MODE
 * firmware opcode.  It does not implement HWMP/MPM as native 802.11s.
 */
static int zt_cfg80211_join_mesh(struct wiphy *wiphy, struct net_device *ndev,
                                 const struct mesh_config *conf,
                                 const struct mesh_setup *setup)
{
    ndev_priv_st *pndev_priv = netdev_priv(ndev);
    nic_info_st *pnic_info = pndev_priv ? pndev_priv->nic : NULL;
    zt_wlan_mgmt_info_t *wlan_mgmt_info;
    zt_wlan_network_t *pcur_network;
    zt_wlan_ssid_t ssid;
    struct ieee80211_channel *chan;
    zt_bool connected = zt_false;

    if (!pnic_info || !setup || !setup->mesh_id || !setup->mesh_id_len)
        return -EINVAL;
    if (setup->mesh_id_len > IW_ESSID_MAX_SIZE)
        return -E2BIG;
    if (pnic_info->is_driver_critical)
        return -EPERM;

    chan = setup->chandef.chan;
    if (!chan)
        return -EINVAL;

    if (zt_local_cfg_get_work_mode(pnic_info) != ZT_ADHOC_MODE)
        zt_local_cfg_set_work_mode(pnic_info, ZT_ADHOC_MODE);

    wlan_mgmt_info = pnic_info->wlan_mgmt_info;
    if (!wlan_mgmt_info)
        return -ENODEV;
    pcur_network = &wlan_mgmt_info->cur_network;

    zt_memset(&ssid, 0, sizeof(ssid));
    ssid.length = setup->mesh_id_len;
    zt_memcpy(ssid.data, setup->mesh_id, setup->mesh_id_len);
    zt_wlan_set_cur_ssid(pnic_info, &ssid);

    pcur_network->bcn_interval = setup->beacon_interval;
    if (!pcur_network->bcn_interval)
        pcur_network->bcn_interval = 100;
    pcur_network->cap_info = ZT_80211_MGMT_CAPAB_IBSS;
    pcur_network->channel = freq_2_ch(chan->center_freq);
    zt_wlan_set_cur_bw(pnic_info, CHANNEL_WIDTH_20);
    zt_mlme_get_connect(pnic_info, &connected);
    if (connected)
        return 0;

    zt_scan_wait_done(pnic_info, zt_true, 1000);
    return zt_mlme_scan_ibss_start(pnic_info, &pcur_network->ssid,
                                   &pcur_network->channel,
                                   ZT_MLME_FRAMEWORK_NETLINK);
}

static int zt_cfg80211_leave_mesh(struct wiphy *wiphy, struct net_device *ndev)
{
    ndev_priv_st *pndev_priv = netdev_priv(ndev);
    nic_info_st *pnic_info = pndev_priv ? pndev_priv->nic : NULL;
    zt_bool connected = zt_false;
...
static nic_info_st *cfg80211_txpower_nic(struct wiphy *wiphy,
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 8, 0))
...
)
{
    ...
    if (wdev && wdev->netdev)
    {
        ndev_priv_st *pndev_priv = netdev_priv(wdev->netdev);
        if (pndev_priv)
            pnic_info = pndev_priv->nic;
    }
#endif
    if (!pnic_info)
        pnic_info = *((nic_info_st **)wiphy_priv(wiphy));
    return pnic_info;
}
...
#endif
{
    ...
    ZT_UNUSED(radio_idx);
#endif
    pnic_info = cfg80211_txpower_nic(wiphy,
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 8, 0))
                                     wdev
#else
                                     NULL
    ...
    if (type == NL80211_TX_POWER_AUTOMATIC)
        req_dbm = 12;
    else if (type == NL80211_TX_POWER_FIXED || type == NL80211_TX_POWER_LIMITED)
        req_dbm = MBM_TO_DBM(mbm);
    else
        return -EINVAL;
#else
    req_dbm = dbm;
    ...
    if (req_dbm < 0 || req_dbm > 30)
        return -EINVAL;

    local_info = (local_info_st *)pnic_info->local_info;
    if (!local_info)
        return -ENODEV;

    CFG80211_INFO("txpower request: type=%d mbm=%d -> %d dBm",
                  (int)type, (int)mbm, (int)req_dbm);

    {
        zt_u8 old_rf_power = local_info->rf_power;
        zt_s8 old_txpower_dbm = local_info->txpower_dbm;

        /* The silicon exposes three calibrated RF tables, not a linear dBm DAC. */
        if (req_dbm <= 7)
            local_info->rf_power = E_RADIO_POWER_LEVEL_L;
        else if (req_dbm <= 15)
            local_info->rf_power = E_RADIO_POWER_LEVEL_M;
        else
            local_info->rf_power = E_RADIO_POWER_LEVEL_H;

        local_info->txpower_dbm = (zt_s8)req_dbm;
        CFG80211_INFO("txpower state before MCU: %d dBm (rf table %u)",
                      (int)local_info->txpower_dbm,
                      (unsigned int)local_info->rf_power);
        ret = zt_mcu_rf_power_init(pnic_info);
        if (ret != ZT_RETURN_OK)
        {
            local_info->rf_power = old_rf_power;
            local_info->txpower_dbm = old_txpower_dbm;
            CFG80211_WARN("set txpower %d dBm failed: %d", req_dbm, ret);
            return -EIO;
        }
    }

    CFG80211_INFO("txpower set: %d dBm (rf table %u)", req_dbm, local_info->rf_power);
    return 0;
}
                                ...
                                zt_s32 *dbm)
{
    ...
    if (!pnic_info || !pnic_info->local_info)
        return -ENODEV;

    local_info = (local_info_st *)pnic_info->local_info;
    *dbm = local_info->txpower_dbm;
    CFG80211_DBG("txpower get: %d dBm", *dbm);
    return 0;
}
...
#ifdef CFG_ENABLE_ADHOC_MODE
    .join_mesh = zt_cfg80211_join_mesh,
    .leave_mesh = zt_cfg80211_leave_mesh,
#endif
